...
sub getTranscriptVariationAdaptor {
    my $tva = $registry->get_adaptor( 'human', 'variation' )
        ;    #,'TranscriptVariation');
    return $tva;
}
...
sub getVariationFeatureAdaptor {
    my $vfa
        = $registry->get_adaptor( 'human', 'Variation', 'VariationFeature' );
    return $vfa;
}
...
sub getComparaMethodLinkSpeciesSetAdaptor {
    my $ss = $registry->get_adaptor( "Compara", "compara",
...
sub getDnaAlignFeatureAdaptor {
    my $daf
        = $registry->get_adaptor( 'Multi', 'compara', 'DnaDnaAlignFeature' );
    return $daf;
}
        ...
        if ( $tran->translateable_seq() ) {
            ...
            for ( my $i = 0; $i < @exons; $i++ ) {
                ...
                if ( $i == 0 ) {
                    ...
                    $ut =~ tr/[a-z]/[A-Z]/;
                    $exon
                        =~ s/$ut//;  #remove 5UTR that is included in 1st exon
                }
                if ( $i == @exons - 1 ) {
                    my $ut = $three;
                    $ut =~ tr/[a-z]/[A-Z]/;
                    $exon
                        =~ s/$ut//; #remove 3UTR that is included in last exon
                }
                $seq = $seq . $exon;
                if ( $i < @exons - 1 ) {
...
sub getAllOrthologousGenes {
    my ( $gene, $member_adaptor, $homology_adaptor,
        @requestedSpeciesFullName )
        = @_;
    my @allGenes = ();
    push( @allGenes, $gene );
...
sub getSpeciesForGenes {
    ...
    foreach my $g (@genes) {
        ...
        my $mb = $member_adaptor->fetch_by_source_stable_id( "ENSEMBLGENE",
            $stableId );
        my $name = $mb->genome_db->name;
...
sub speciesMatch {
    ...
    for ( my $i = 0; $i < @requestedSpecies; $i++ ) {
        ...
        if ( ( $querySpecies =~ /$requestedSpecies[$i]/i ) )
        {    #||($altName=~/$requestedSpecies[$i]/i)){
            return 1;
        }
    }
    return 0;
...
sub getVariationDetailsForTranscripts {
    ...
    if ( defined $humanDbVariation ) {
        ...
        for ( my $m = 0; $m < @transcripts; $m++ ) {
            my $trvs = $trv_adaptor->fetch_all_by_Transcripts(
                [ $transcripts[$m] ] );
            foreach my $tv ( @{$trvs} ) {
           ...
           #4 lines above are not efficient as multiple mysql connections made
                ...
                my @coords
                    = $transcripts[$m]
                    ->genomic2cdna( $varGenomicStart, $varGenomicEnd,
                    $varStrand );
...
sub getVariationDetailsForTranscripts2 {
    ...
    for ( my $m = 0; $m < @transcripts; $m++ ) {
        my $trvs = $trv_adaptors[$m]
            ->fetch_all_by_Transcripts( [ $transcripts[$m] ] );
        foreach my $tv ( @{$trvs} ) {
...
#push(@boundaries,"sequenceName\,columnNumber\,residueNumber\,number\,RCOL\,GCOL\,BCOL\,SYMBOL\,name\,5utrEnd\,exonEnd\n");
    while ( my $tran = shift(@transcripts) ) {
        ...
        if ( defined $five_utr ) {
            ...
            $exLength = $fiveLength
                ; #### added line by Qing so that the output number matches the residue number of the enxons. ###
        }
