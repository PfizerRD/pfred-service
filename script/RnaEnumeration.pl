...
my $usage
    = "$0 -p primaryID -l oligoLength -f \"fastainputfile.fa containing transcript sequences any additional .fa files\" -o oligoOutFile \n";
@ARGV > 1 || die $usage;
...
my @oligos = &enumerateSeq( $primarySeq, $oligoLen )
    ;    #@oligo[dnaSense][rnaSense][rnaAntisense]
...
# disable bowtie for the moment --dmitri
...
if ($oligoOutFile) {
  open( $fh_out, ">$oligoOutFile" )
      || die "Can't write to file $oligoOutFile\n$usage";
}
else {
  $fh_out = *STDOUT;
}
...
foreach my $tr (@transcriptNames) {
  print $fh_out ",${tr}_match", ",${tr}_position";
}
print $fh_out "\n";
my ( $name, $start, $end );
for ( my $i = 1; $i <= @{ $oligos[0] }; $i++ ) {
  ...
  $end   = $i + $oligoLen - 1;
  print $fh_out
      "$name,$start,$end,$oligoLen,$oligos[0]->[$i-1],$oligos[1]->[$i-1],$oligos[2]->[$i-1],$primaryID";
  for ( my $j = 0; $j < @transcriptNames; $j++ ) {
    ...
    print $fh_out ",@{$countHash->{$name}{$tr}},@{$posHash->{$name}{$tr}}";
  }
  print $fh_out "\n";
}


close $fh_out if $oligoOutFile;
...
sub bowtieSearch {
  ...
  $BOWTIE_option
      .= ( ( $type =~ /antisense/i ) ) ? " --nofw -f" : " --norc -f";

  my $bowtie_out     = "${bowtie_in}_${db}.out";
  my $bowtie_noAlign = "${bowtie_in}_${db}.noAlign";
  print STDERR (
    "set BOWTIE_INDEXES $BOWTIE_INDEXES; $BOWTIE $BOWTIE_option --un $bowtie_noAlign $db $bowtie_in > $bowtie_out;\n"
  );
  system(
    "set BOWTIE_INDEXES $BOWTIE_INDEXES; $BOWTIE $BOWTIE_option --un $bowtie_noAlign $db $bowtie_in > $bowtie_out;"
  );
...
sub parseBowtieOutput {
  ...
  while (<OUT>) {
    ...
    if ( !defined $count{$name}{$transcript} ) {
      ...
      $position{$name}{$transcript}
          = [ $da[3] + 1 ];    # bowtie reported position starting at 0
...
sub buildBowtieInputFastaFile {
  ...
  open( OLIGO, "<$seqfile" )
      || die
      "buildBowtieInputFastaFile() error: can't open  oligo input file $seqfile\n\n$usage";
  open( FA, ">$bowtie_in" )
      || die
      "buildBowtieInputFastaFile() error: can't write file $bowtie_in\n\n$usage";
  if ( $format =~ /tab/ ) {
    ...
    while (<OLIGO>) {
      if ( $_ =~ /^>/ ) {
        ...
        print STDERR
            "Error from buildBowtieInputFastaFile: $seqfile not in right format\n";
      }
    }
  }
  close FA;
...
sub readSeq {
  ...
  if ( $str !~ /^\s*[AUCGTX\s]+\s*$/i ) {
    print STDERR
        "Warning Sequence contains nonstandard (AUCGTX) residues:\n$str\n";
  }
  return $str;
}
...
sub getReversComplement {
  ...
  $compStr = reverse($str);
  ( $type =~ /DNA/i )
      ? $compStr =~ tr/CGAUT/GCTAA/
      : $compStr =~ tr/CGAUT/GCUAA/;
